  ...
  if (isset($_POST['simpan'])) { // create Data
    ...
    if ($nim && $nama && $no_kipk && $fakultas && $prodi && $angkatan) {
      ...
      if ($op == 'edit') { // update Data
        ...
        $checkNim = mysqli_query($koneksi, "select nim from penerima where nim='$nim'");
        $checkNo = mysqli_query($koneksi, "select no_kipk from penerima where nim='$nim'");
        if (mysqli_num_rows($checkNim) > 0 || mysqli_num_rows($checkNo) > 0) {
          $error = "Nim Atau No KIP Duplikat";
        } else {
